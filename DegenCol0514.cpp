...
{
	int i, j, l, end, rend, pa;
	if (u == K)
	{
		(*n)++;
		return;
	}
	if (u == K + K * partiteSize - 1) {
	...
	// {
	// 	pa = true;
	// 	for (j = 0; j < partiteSize; ++j)
	// 	{
	// 		if (abs(cnt[i] - cnt[j]) > A)
	// 		{
	// 			pa = false;
	// 			break;
	// 		}
	...
	{
		// if (cnt[v2m[Candidate[i]]] > B) continue;
		if (u - cnt[v2m[Candidate[i]]] == visCNT[Candidate[i]])
		{
			// if (cnt[v2m[Candidate[i]]] + 1 > 1 + K + A) continue;
...
void kclique(unsigned l, specialsparse *g, unsigned long long *n) {
	...
	if (l == 2) {
		...
		for (i = 0; i < g->ns[2]; i++) {//list all edges
			...
			rend = g->rcd[u] + g->rd[u];
			bool go = true;
			for (ne = g->cd[u]; ne < end; ne++) visCNT[g->adj[ne]]++; // COLOR CSR
			...
			for (j = g->cd[u]; j < end; j++) {
				...
				rend_ = g->rcd[minV] + g->rd[minV];
				memset(partiteCNT, 0, sizeof(int) * partiteSize);
				// 枚举点的邻居 neighbor
				...
				{
					cnt[I] = 1;
					upperK = min(partiteCNT[I], upperK);
				}
				K = upperK;
				// if (K == partiteSize + 1)
...
int main(int argc, char** argv) {
	...
	partiteCNT = new int [partiteSize];
	t2 = time(NULL);
	...
	fs << argv[3] << ",comb," << g->n << "," << g->e << "," << (int)k << "," << n << "," << Duration(ti) << "," << m2 - m1 << "," << graphSize - m1 << "," << A << "," << B << endl;
	t2 = time(NULL);
	printf("- Time = %ldh%ldm%lds\n", (t2 - t1) / 3600, ((t2 - t1) % 3600) / 60, ((t2 - t1) % 60));
	t1 = t2;
