	...
	if (l == 2) {
		for (i = 0; i<g->ns[2]; i++) {//list all edges
			...
			for (j = g->cd[u]; j<end; j++) {
				v = g->adj[j];
				if (v<u) {
					if (CNT[v2m[v]] + 1 > MaxCnt) continue;
					CNT[v2m[v]]++;
					...
					if (ok)
					{
						for (int kd = 0; kd < partiteSize; kd++)
						{
							for (int kd2 = kd + 1; kd2 < partiteSize; kd2++)
								if (abs(CNT[kd] - CNT[kd2]) > A)
								{
									ok = false;
									break;
								}
...
int main(int argc, char** argv) {
	...
	fs << argv[3] << ",base," << g->n << "," << g->e << "," << (int)k << "," << n << "," << Duration(ti) << "," << m2 - m1 << "," << graphSize - m1 << endl;
